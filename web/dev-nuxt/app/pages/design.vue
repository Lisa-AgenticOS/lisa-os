...
// libs/lisa_ui/lib/lisa_ui.dart, docs/notes/design-direction.md, ADR-0010/0014.
...
<template>
  <div>
    <header class="pagehead">
      ...
      <p class="lede">Lisa's direction is elementary-inspired: restrained typography, quiet color, humane defaults, one visual voice (recorded in <a :href="`${repo}/blob/main/docs/notes/design-direction.md`">docs/notes/design-direction.md</a>). Tokens first — a single token source is the design language; shell CSS, GTK, Qt, and the Flutter kit all read from it.</p>
    </header>

    <section class="sec anchor">
      <h2>The violet ramp</h2>
      <p>Everything brand-colored derives from the Lisa violet, <code>#6D45C9</code> — the same seed the GDM greeter, the wordmark accents, and <code>lisa_ui</code>'s <code>ColorScheme.fromSeed</code> use. The ramp as this site ships it (<code>--color-lisa-*</code>):</p>
      <div class="ramp">
    ...
    <section class="sec anchor">
      <h2>Type: Rubik</h2>
      <p>Rubik is the UI face — shipped in the OS image as the system font (gschema override) and named as <code>lisa_ui</code>'s default <code>fontFamily</code>. It is deliberately <em>not</em> bundled in the Flutter kit and there is no google_fonts dependency: the family name resolves against the OS-installed font and falls back to the platform default sans when absent. Monospace surfaces (code, dates, ledger entries) use the platform mono stack with tabular numerals.</p>
    </section>

    <section class="sec anchor">
      <h2>The lisa_ui widget kit</h2>
      <p><code>libs/lisa_ui</code> is the single import for Lisa Flutter apps (ADR-0014): a curated re-export of the Material vocabulary plus the pieces Material does not have. From <a :href="`${repo}/blob/main/libs/lisa_ui/lib/lisa_ui.dart`">lisa_ui.dart</a>:</p>
      <div class="tbl"><table>
      ...
      <div class="note">Status: the kit wraps <code>package:flutter/material.dart</code> today (ADR-0014 phase 1); phase 2 swaps the backend to a vendored, re-themed Material fork with no app-facing API change. The kit is widget-tested and ready; the Forge app lane that consumes it is M6.</div>
    </section>

    <section class="sec anchor">
      <h2>Principles</h2>
      <ul>
        <li><strong>Tokens first.</strong> One token source is the design language — shell CSS, GTK/libadwaita, Qt, and <code>lisa_ui</code> all read it. One voice across every surface.</li>
        <li><strong>Identity where freedom is total.</strong> First-party apps and <code>lisa_ui</code> carry the look first — our widgets, our rules; the GNOME base is kept for portal maturity.</li>
