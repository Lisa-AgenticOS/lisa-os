...
| "Write with Siri anywhere you type," system-wide even in third-party apps | Deep private toolkit hooks only Apple can do | **Three-layer Writing Tools (§5.7.3):** toolkit modules for GTK/Qt, an input-method (fcitx5) layer that *can* reach everything including Electron and terminals, and an AT-SPI/portal fallback. **On the reference device it reaches nothing yet:** fcitx5 reports `Using Wayland native input method protocol: 0`, so the addon loads and is never given a key (#208, §5.7.3). |
...
**Repo:** `libs/{forge-harness, lisa.sdk}`.
